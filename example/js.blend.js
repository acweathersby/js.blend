...
var jsblend = (function(USE_WEBWORKER, UNUSED_NETWORK_PORT, unzipper) {
    ...
    function worker_code() {
        ...
        */
        ...
        function uIntProp(offset, Array_match, length) {
            return {
                get: function() {
                    return (Array_match > 1) ? 
        ...
        var uShortProp = (offset, Array_match, length) => {
            return {
                get: function() {
                    return (Array_match > 1) ? 
        ...
        function charProp(offset, Array_match, length) {
            return {
                get: function() {
                    return (Array_match > 1) ? toString(this.__blender_file__.AB, this.__data_address__ + offset, this.__data_address__ + offset + length) : this.__blender_file__.byte[(this.__data_address__ + offset)];
                },
                set: function(byte) {
                    if (Array_match > 1) {
                        var string = byte + "",
        ...
        function pointerProp(offset, Array_match, length){
            return { 
                get : function(){
        ...
        function compileProp(obj, name, type, offset, array_size, IS_POINTER, pointer_size, length){
            ...
            if (!IS_POINTER) {
                ...
                Object.defineProperty(obj, name, pointerProp(offset, array_size, (pointer_size > 4) ? length >> 3 : length >> 2));
                ...
                if (struct) {
                    ...
                    for (var i = 0; i < struct.length; i += 3) {
                        ...
                        if ((match = _name.match(/(\*?)(\*?)(\w+)(\[(\w*)\])?(\[(\w*)\])?/))) {
                            ...
                            if(match[2]){
                                console.log(_name, type, original_name, name)
                                PointerToArray = true;
                            }
                            ...
                            if(match[4]){
                                if(match[6]){
                                    ...
                                    Array_match = parseInt(match[7]);
                                }else{
                                    Array_match = parseInt(match[5]);
                                }  
                            }
                            array_length = Array_match * length;
                        ...
                        if(PointerToArray){
                            ...
                            for(var j = 0; j < Suparray_match; j++){
                                ...
                                offset = compileProp(blen_struct.prototype, array_name_, type, offset, Array_match, Pointer_Match, pointer_size, array_length);
                            }
                            ...
                            offset = compileProp(blen_struct.prototype, _name, type, offset, Array_match, Pointer_Match, pointer_size, length);    
                        }
                    }
